# =============================================================================
# full_production_near_storm_tcwv_multi_rad_4K_HKU.py
# =============================================================================
#
# PURPOSE:
# Process d4PDF +4K warming scenario (netCDF version) to compute:
...
#
# Adapted from collaborator's netCDF loading code.
# AUTHOR: Jianan Chen (for Nature Geoscience)
...
# ============================ USER CONFIGURATION ============================
OUTPUT_DIR = './4K_MR_HKU_TCWV'                    
TRACK_PATH_TEMPLATE = './Tracks/MRI_HFB_4K_MR_m{ensemble_id:03d}.nc'

Q_PATH_TEMPLATE = (
    #'nc.x/'
    #'HFB_4K_MR_m{ensemble_id:03d}/d_monit_a_nc/{year}/'
    #'q.nc'
)

START_YEAR = 2051
END_YEAR   = 2110          # change to 2110 when ready
ensemble_id_start = 101
ensemble_id_end   = 101    # change to 116 when ready

# Radii for cumulative averaging
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(OUTPUT_DIR, "production_4K_TCWV_multi_rad.log")),
...
# ====================== VALIDATED TCWV FUNCTION ======================
def calculate_full_tcwv(q_month, t_idx, surface_pres_hpa):
    """Full-grid TCWV (kg m⁻²) from track-central PRES to model top"""
    g = 9.80665
...
def load_cyclone_tracks_per_year(track_file: str, year: int) -> pd.DataFrame:
    ...
    try:
        with xr.open_dataset(track_file) as ds:
            ...
            logging.info(f"Loaded {len(df)} valid TC points for year {year}")
            return df
    except Exception as e:
        logging.error(f"Failed to load tracks from {track_file}: {e}")
        return pd.DataFrame()

# ============================= DATA LOADING (collaborator's netCDF style) =============================
def load_month_q(q_path: str, year: int, month: int):
    """Load Q from netCDF (collaborator's exact loading method)"""
    try:
        ds = xr.open_dataset(q_path)
        da = ds['q']
        q_raw = da.sel(lat=slice(-60, 60), time=da.time.dt.month.isin([month])).values
        ds.close()
        nt = days_in_month(year, month) * 4
        expected_size = nt * 12 * 97 * 288
        if q_raw.size != expected_size:
            logging.warning(f"Size mismatch for {q_path}: expected {expected_size}, got {q_raw.size}")
            return None
...
def process_ensemble(ensemble_id: int):
    ...
    for year in range(START_YEAR, END_YEAR + 1):
        ...
        for row in pbar:
            ...
            q_path = Q_PATH_TEMPLATE.format(ensemble_id=ensemble_id, year=year)
            ...
            try:
                ...
                logging.warning(f"Time {row.ISO_TIME} not found")
                continue
        ...
        if results:
            ...
            out_file = os.path.join(OUTPUT_DIR, f'4K_MR_TCWV_multi_rad_{year}_EN_{ensemble_id:03d}.csv')
            df_out.to_csv(out_file, index=False)
            logging.info(f"Saved {len(df_out)} rows with multi-radius + annulus TCWV → {out_file}")

# ============================= MAIN EXECUTION =============================
if __name__ == '__main__':
    logging.info("=== Starting +4K TCWV multi-radius + 600-800 km annulus production (netCDF) ===")
    ens_list = []
    for i in range(ensemble_id_start, ensemble_id_end + 1):
        track_path = TRACK_PATH_TEMPLATE.format(ensemble_id=i)
        if os.path.exists(track_path):
            ens_list.append(i)
    logging.info(f"Found {len(ens_list)} ensembles: {ens_list}")
    ...
    logging.info("=== PRODUCTION COMPLETE ===")
    print("\nAll processing finished.")
    print(f"Output directory: {OUTPUT_DIR}")
    print("Check production_4K_TCWV_multi_rad.log for details.")