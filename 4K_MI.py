...
#
# PURPOSE:
# Process d4PDF +4K warming scenario (HFB_4K_MI) to compute area-averaged
# Relative Humidity (RH) and Saturation Deficit (SD) within 500km of
# tropical cyclone centers at multiple pressure levels.
#
# Track files must be placed in ./Tracks (MII_HFB_4K_MI_mXXX.nc)
...
OUTPUT_DIR = '/home/jchenfw/d4PDF/4K_MI_HKU'   # Please replace an absolute path, not relative path, I tried relative path, it does not work. Thanks!

# Path template for tropical cyclone track files
TRACK_PATH_TEMPLATE = '/home/jchenfw/d4PDF/Tracks/MII_HFB_4K_MI_m{ensemble_id:03d}.nc'  # Please replace an absolute path, not relative path, I tried relative path, it does not work. Thanks!

# Path templates for specific humidity (Q) and temperature (T) binary files
Q_PATH_TEMPLATE = (
    '/home/jchenfw/d4PDF/Data/'
    'd4PDF_GCM/HFB_4K_MI/m{ensemble_id:03d}/atm_snp_6hr_1.25deg_HFB_4K_MI_'   # please replace this with actually path, thanks a lot!
...
T_PATH_TEMPLATE = (
    ...
    'd4PDF_GCM/HFB_4K_MI/m{ensemble_id:03d}/atm_snp_6hr_1.25deg_HFB_4K_MI_' 
...
def process_ensemble(ensemble_id: int):
    ...
    for year in range(START_YEAR, END_YEAR + 1):
        ...
        if results:
            ...
            out_file = os.path.join(OUTPUT_DIR, f'4K_MI_{year}_EN_{ensemble_id:03d}.csv')
...
if __name__ == '__main__':
    ...
    # Auto-detect available MI ensembles
    ens_list = []
    ...
    logging.info(f"Found {len(ens_list)} +4K MI ensembles: {ens_list}")
